...
class Dropdown extends React.Component {
  constructor(props) {
    super(props);
    this.state = {countries: [],states: [], sectors: [], delegation: '',gouvernerat: '', secteur: ''};
  ...
  changedSector(event){
	this.setState({ secteur: {
		name: event.target.value, 
		code: this.state.sectors.filter((secteur) => {return secteur.name === event.target.value})[0].code
	  } }, () => {
  ...
  changeState(event) {
	...
	this.setState({sectors: this.state.states.find((state) => state.name === event.target.value).secteur});
